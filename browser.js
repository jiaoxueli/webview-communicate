...
// const appex = require('electron').remote.require('./appex.js');
const appex = require('./appex.js');
console.log(appex);
...
onload = function() {
  ...
  document.querySelector('#test').onclick = function(e){
    e.preventDefault();
    webview.loadURL("file://"+__dirname+"/test.html");
  };

  document.querySelector('#refresh').onclick = function(e){
    // ipcRenderer.send('close-eccp');
    // ipcRenderer.send('ping');
   // appex.refresh();
    webview.executeJavaScript('window.appex.onRefresh()');
  };
  
  webview.addEventListener('ipc-message',event=>{
    console.log(event.channel);
    if(event.channel == 'onrefresh'){
